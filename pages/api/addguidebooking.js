...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        try {
            ...
            const guideBookingCollection = db.collection('guidebooking');
            // console.log(req.body)
            const { name, number, email, noOfDays, date,gid,type,price} = req.body;
            ...
            const result = await guideBookingCollection.insertOne(bookingData);
            ...
            const customerEmailOptions = {
                ...
                <html>
                    ...
                    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                        ...
                        <p>We’ve received your guide booking request and will connect with you shortly on WhatsApp (<strong>${number}</strong>) for confirmation.</p>
...
//             };
    ...
    >
        Customer Contact
            ...
            res.status(201).json({ message: 'Guide booking added and emails sent successfully!' });
        } catch (error) {
            console.error('Error adding Guide booking:', error);
            res.status(500).json({ error: 'Failed to add Guide booking', details: error.message });
        }
    } else {
